...
$$ \begin{aligned} \int(\ln x)^{n} d x &=(\ln x)^{n} x-\int x d(\ln x)^{n} \\\\\\\\ &=(\ln x)^{n} x-n \int x(\ln x)^{n-1} \frac{1}{x} d x \\\\\\\\ &=(\ln x)^{n} x-n \int(\ln x)^{n-1} d x \end{aligned} $$

例2. 同上，$$ \begin{aligned} \int x^{n} e^{x} d x &=\int x^{n} d e^{x} \\ &=x^{n} e^{x}-\int e^{x} d x^{n} \\ &=x^{n} e^{x}-n \int e^{x} x^{n-1} d x \end{aligned} $$

形象的解释分部积分就是：

![图片44.png](https://upload.wikimedia.org/wikipedia/commons/thumb/1/16/Integration_by_parts_v2.svg/1024px-Integration_by_parts_v2.svg.png)

$$\int_{x_{1}}^{x_{2}} y \mathrm{d} x+\int_{y_{1}}^{y_{2}} x \mathrm{d} y=\left.x y(x)\right|_{x_{1}} ^{x_{2}}$$
